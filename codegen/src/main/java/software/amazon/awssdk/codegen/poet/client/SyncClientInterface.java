...
public final class SyncClientInterface implements ClassSpec {
    ...
    private List<MethodSpec> operationMethodSpec(OperationModel opModel) {
        ...
        methods.add(operationMethodSignature(model, opModel, opModel.getMethodName())
        ...
        if (model.getPaginators().keySet().contains(opModel.getOperationName())) {
            ...
            if (opModel.getInputShape().isSimpleMethod()) {
                methods.add(simpleMethodForPaginatedOperation(opModel));
            }

            methods.add(operationMethodSignature(model, opModel, PaginatorUtils.getSyncMethodName(opModel.getMethodName()))
    ...
    private MethodSpec simpleMethodForPaginatedOperation(OperationModel opModel) {
        ClassName requestType = ClassName.get(model.getMetadata().getFullModelPackageName(),
                opModel.getInput().getVariableType());

        return operationSimpleMethodSignature(model, opModel, PaginatorUtils.getSyncMethodName(opModel.getMethodName()))
                .returns(poetExtensions.getResponseClassForPaginatedOperation(opModel.getOperationName()))
                .addModifiers(Modifier.DEFAULT)
                .addStatement("return $L($T.builder().build())",
                        PaginatorUtils.getSyncMethodName(opModel.getMethodName()), requestType)
                .build();
    }
